...
- マージ済みリストを severity 降順で出力する。ソート順序（数値順）:
  1. `critical` (最高)
  2. `high`
  3. `medium`
  4. `low` (最低)
  - **注意**: 文字列のアルファベット順ではソートしないこと（`"low"` < `"medium"` が逆転するため）。
  - **タイブレーク**: 同一 severity 内ではサブエージェント出力の原順を保持する（安定ソート）。
...
各サブスキルに渡す入力:
- **code**: 対象ソースコード（必須）
- **scope**: スコープ定義（必須、Step 1 で定義）
- **context.prior_findings**: 先行エージェントの findings（任意、参照のみ）
  - 検出のヒントとして使用。先行 findings がなくても独立に検出すること。

### エラーハンドリング

- サブエージェントが失敗または空 findings を返した場合、残りのサブエージェントは続行する（中断しない）。
- 空 findings は正当な結果（指摘なし）として扱う。
